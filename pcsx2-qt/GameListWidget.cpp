...
void GameListWidget::loadTableViewColumnVisibilitySettings()
{
  ...
  for (int column = 0; column < GameListModel::Column_Count; column++)
  {
    const bool visible = DEFAULT_VISIBILITY[column]; /*m_host_interface->GetBoolSettingValue(
...
void GameListWidget::loadTableViewColumnSortSettings()
{
  ...
  const GameListModel::Column sort_column = DEFAULT_SORT_COLUMN; /*
  ...
  const bool sort_descending = DEFAULT_SORT_DESCENDING;          /*
