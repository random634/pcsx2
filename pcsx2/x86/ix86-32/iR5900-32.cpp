...
void recompileNextInstruction(int delayslot)
{
	...
	//Log::Console.warning("opcode name = {:s}, it's cycles = {:d}\n",opcode.Name,opcode.cycles);
	...
	switch (PageType)
	{
		...
		case ProtMode_Manual:
			...
			if (!contains_thread_stack && manual_counter[inpage_ptr >> 12] <= 3)
			{
				...
				Log::EERecPerf.info(LogStyle::CompatibilityGray, "Manual block @ {:08X} : size ={:>3d}  page/offs = 0x{:05X}/0x{:03X}  inpgsz = {:d}  clearcnt = {:d}\n",
	...
	while (1)
	{
		...
		if (i != startpc) // Block size truncation checks.
		{
			if ((i & 0xffc) == 0x0) // breaks blocks at 4k page boundaries
			{
				...
				Log::EERecPerf.debug("Pagesplit @ {:08X} : size={:d} insts\n", startpc, (i - startpc) / 4);
				break;
			}
