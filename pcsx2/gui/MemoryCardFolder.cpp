...
bool FolderMemoryCard::AddFile(MemoryCardFileEntry* const dirEntry, const wxString& dirPath, const EnumeratedFileEntry& fileEntry, MemoryCardFileMetadataReference* parent)
{
	...
	{
		Log::Console.info("(FolderMcd) Could not open file: {:s}\n", relativeFilePath.GetFullPath());
		return false;
	}
}
...
void FolderMemoryCard::Flush()
{
	...
	Log::Console.info("(FolderMcd) Writing data for slot {:d} to file system...\n", m_slot);
	...
	{
		Log::Console.warning("(FolderMcd) Aborting flush of slot {:d}, emulation was interrupted during save process!\n", m_slot);
		return;
	}
	...
	Log::Console.info("(FolderMcd) Done! Took {:d} ms.\n", timeFlushEnd - timeFlushStart);
