...
AsyncFileReader* MultipartFileReader::DetectMultipart(AsyncFileReader* reader)
{
	...
	{
		Log::Console.info(LogStyle::CompatibilityBlue, "isoFile: multi-part ISO detected.  {:d} parts found.\n", multi->m_numparts);
...
void MultipartFileReader::FindParts()
{
	...
	Log::Console.debug(LogStyle::CompatibilityBlue, "isoFile: multi-part {:s} detected...\n", curext.Upper());
	...
	for (; i < MaxParts; ++i)
	{
		...
		Log::Console.debug(LogStyle::CompatibilityBlue, "    blocks {:d} - {:d} in: {:s}\n",
			thispart->start, thispart->end, nameparts.GetFullPath()
		);
