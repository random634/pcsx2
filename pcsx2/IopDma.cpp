...
static void __fastcall psxDmaGeneric(u32 madr, u32 bcr, u32 chcr, u32 spuCore)
{
	...
	switch (chcr)
	{
		case 0x01000201: //cpu to spu2 transfer
			Log::IOP::DMAHW.debug("*** DMA {:d} - mem2spu *** {:x} addr = {:x} size = {:x}\n", dmaNum, chcr, madr, bcr);
			if (dmaNum == 7)
		...
		case 0x01000200: //spu2 to cpu transfer
			Log::IOP::DMAHW.debug("*** DMA {:d} - spu2mem *** {:x} addr = {:x} size = {:x}\n", dmaNum, chcr, madr, bcr);
			if (dmaNum == 7)
		...
		default:
			Log::Console.error("*** DMA {:d} - SPU unknown *** {:x} addr = {:x} size = {:x}\n", dmaNum, chcr, madr, bcr);
			break;
	}
}
...
void psxDma6(u32 madr, u32 bcr, u32 chcr)
{
	...
	Log::IOP::DMAHW.debug("*** DMA 6 - OT *** {:x} addr = {:x} size = {:x}\n", chcr, madr, bcr);
	...
	{
		// Unknown option
		Log::IOP::DMAHW.debug("*** DMA 6 - OT unknown *** {:x} addr = {:x} size = {:x}\n", chcr, madr, bcr);
	}
...
void psxDma8(u32 madr, u32 bcr, u32 chcr)
{
	...
	switch (chcr & 0x01000201)
	{
		case 0x01000201: //cpu to dev9 transfer
			Log::IOP::DMAHW.debug("*** DMA 8 - DEV9 mem2dev9 *** {:x} addr = {:x} size = {:x}\n", chcr, madr, bcr);
			DEV9writeDMA8Mem((u32*)iopPhysMem(madr), size);
			break;

		case 0x01000200: //dev9 to cpu transfer
			Log::IOP::DMAHW.debug("*** DMA 8 - DEV9 dev9mem *** {:x} addr = {:x} size = {:x}\n", chcr, madr, bcr);
			DEV9readDMA8Mem((u32*)iopPhysMem(madr), size);
			break;

		default:
			Log::IOP::DMAHW.debug("*** DMA 8 - DEV9 unknown *** {:x} addr = {:x} size = {:x}\n", chcr, madr, bcr);
			break;
	}
...
{
	Log::SIF.debug("IOP: dmaSIF0 chcr = {:x}, madr = {:x}, bcr = {:x}, tadr = {:x}\n", chcr, madr, bcr, HW_DMA9_TADR);
...
{
	Log::SIF.debug("IOP: dmaSIF1 chcr = {:x}, madr = {:x}, bcr = {:x}\n", chcr, madr, bcr);
