...
{
	//Log::Console.debug("DEV9: *ATA_HARD RESET\n");
	ResetBegin();
	ResetEnd(true);
}
...
u16 ATA::Read16(u32 addr)
{
	switch (addr)
	{
		case ATA_R_DATA:
			return ATAreadPIO();
		case ATA_R_ERROR:
			//Log::Console.debug("DEV9: *ATA_R_ERROR 16bit read at address {:x}, value {:x}, Active {:s}\n", addr, regError, (GetSelectedDevice() == 0) ? "True" : "False");
			if (GetSelectedDevice() != 0)
				return 0;
			return regError;
		case ATA_R_NSECTOR:
			//Log::Console.debug("DEV9: *ATA_R_NSECTOR 16bit read at address {:x}, value {:x}, Active {:s}\n", addr, nsector, (GetSelectedDevice() == 0) ? "True" : "False");
			if (GetSelectedDevice() != 0)
				return 0;
		...
		case ATA_R_SECTOR:
			...
			if (!regControlHOBRead)
				...
				return regSectorHOB;
		case ATA_R_LCYL:
			//Log::Console.debug("DEV9: *ATA_R_LCYL 16bit read at address {:x}, value {:x}, Active {:s}\n", addr, regLcyl, (GetSelectedDevice() == 0) ? "True" : "False");
			if (GetSelectedDevice() != 0)
				return 0;
			if (!regControlHOBRead)
				return regLcyl;
...
void ATA::Write16(u32 addr, u16 value)
{
	...
	{
		case ATA_R_FEATURE:
			//Log::Console.debug("DEV9: *ATA_R_FEATURE 16bit write at address {:x}, value {:x}\n", addr, value);
			ClearHOB();
			regFeatureHOB = regFeature;
			regFeature = (u8)value;
			break;
		case ATA_R_NSECTOR:
			//Log::Console.debug("DEV9: *ATA_R_NSECTOR 16bit write at address {:x}, value {:x}\n", addr, value);
			ClearHOB();
			regNsectorHOB = regNsector;
			regNsector = (u8)value;
			break;
		case ATA_R_SECTOR:
			//Log::Console.debug("DEV9: *ATA_R_SECTOR 16bit write at address {:x}, value {:x}\n", addr, value);
			ClearHOB();
			regSectorHOB = regSector;
			regSector = (u8)value;
			break;
		case ATA_R_LCYL:
			//Log::Console.debug("DEV9: *ATA_R_LCYL 16bit write at address {:x}, value {:x}\n", addr, value);
			ClearHOB();
			regLcylHOB = regLcyl;
			regLcyl = (u8)value;
			break;
		case ATA_R_HCYL:
			//Log::Console.debug("DEV9: *ATA_R_HCYL 16bit write at address {:x}, value {:x}\n", addr, value);
			ClearHOB();
			regHcylHOB = regHcyl;
			regHcyl = (u8)value;
			break;
		case ATA_R_SELECT:
			//Log::Console.debug("DEV9: *ATA_R_SELECT 16bit write at address {:x}, value {:x}\n", addr, value);
			regSelect = (u8)value;
			//bus->ifs[0].select = (val & ~0x10) | 0xa0;
			//bus->ifs[1].select = (val | 0x10) | 0xa0;
			break;
		case ATA_R_CONTROL:
			//Log::Console.debug("DEV9: *ATA_R_CONTROL 16bit write at address {:x}, value {:x}\n", addr, value);
			...
			if ((value & 0x4) != 0)
			{
				Log::Console.debug("DEV9: *ATA_R_CONTROL RESET\n");
				ResetBegin();
				ResetEnd(false);
			}
			if ((value & 0x80) != 0)
			...
			break;
		case ATA_R_CMD:
			//Log::Console.debug("DEV9: *ATA_R_CMD 16bit write at address {:x}, value {:x}\n", addr, value);
			regCommand = value;
