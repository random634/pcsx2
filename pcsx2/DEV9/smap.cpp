...
void tx_process()
{
	...
	for (fc = 0; fc < cnt; fc++)
	{
		...
		else
		{
			...
			Log::Console.debug("DEV9: Sending Packet from base {:x}, size {:d}\n", base, pbd->length);
			...
			*/
			...
			if (base + pbd->length > 16384)
			{
				...
				memcpy(pk.buffer + was, dev9.txfifo, pbd->length - was);
				Log::Console.debug("DEV9: Warped read, was={:d}, sz={:d}, sz-was={:d}\n", was, pbd->length, pbd->length - was);
			}
			else
			{
...
void emac3_write(u32 addr)
{
	...
	switch (addr)
	{
		case SMAP_R_EMAC3_MODE0_L:
			//Log::Console.debug("DEV9: SMAP: SMAP_R_EMAC3_MODE0 write {:x}\n", value);
		...
		case SMAP_R_EMAC3_TxMODE0_L:
			//Log::Console.debug("DEV9: SMAP: SMAP_R_EMAC3_TxMODE0_L write {:x}\n", value);
			//Process TX  here ?
		...
		case SMAP_R_EMAC3_TxMODE1_L:
			//Log::Console.debug("DEV9: SMAP_R_EMAC3_TxMODE1_L 32bit write {:x}\n", value);
			if (value == 0x380f0000)
			{
		...
		case SMAP_R_EMAC3_STA_CTRL_L:
			//Log::Console.debug("DEV9: SMAP: SMAP_R_EMAC3_STA_CTRL write {:x}\n", value);
			{
				if (value & (SMAP_E3_PHY_READ))
				{
					...
					//Log::Console.debug("DEV9: phy_read {:d}: {:x}\n", reg, val);
				...
				if (value & (SMAP_E3_PHY_WRITE))
				{
					...
					//Log::Console.debug("DEV9: phy_write {:d}: {:x}\n", reg, val);
					dev9.phyregs[reg] = val;
				}
			}
			break;
		default:
			Log::Console.debug("DEV9: SMAP: emac3 write  {:x}={:x}\n", addr, value);
			break;
	}
...
u8 smap_read8(u32 addr)
{
	switch (addr)
	{
		case SMAP_R_TXFIFO_FRAME_CNT:
			Log::Console.debug("DEV9: SMAP_R_TXFIFO_FRAME_CNT read 8\n");
			break;
		case SMAP_R_RXFIFO_FRAME_CNT:
			Log::Console.debug("DEV9: SMAP_R_RXFIFO_FRAME_CNT read 8\n");
			break;
		...
		default:
			Log::Console.debug("DEV9: SMAP : Unknown 8 bit read @ {:X},v={:X}\n", addr, dev9Ru8(addr));
			return dev9Ru8(addr);
	}

	Log::Console.debug("DEV9: SMAP : error , 8 bit read @ {:X},v={:X}\n", addr, dev9Ru8(addr));
	return dev9Ru8(addr);
}
...
u16 smap_read16(u32 addr)
{
	...
	if (addr >= SMAP_BD_TX_BASE && addr < (SMAP_BD_TX_BASE + SMAP_BD_SIZE))
	{
		...
		case 0: // ctrl_stat
			hard = dev9Ru16(addr);
			//Log::Console.debug("DEV9: TX_CTRL_STAT[{:d}]: read {:x}\n", (addr - SMAP_BD_TX_BASE) / 8, hard);
			if(dev9.bd_swap)
				return (hard<<8)|(hard>>8);
			return hard;
		case 2: // unknown
			hard = dev9Ru16(addr);
			//Log::Console.debug("DEV9: TX_UNKNOWN[{:d}]: read {:x}\n", (addr - SMAP_BD_TX_BASE) / 8, hard);
			if(dev9.bd_swap)
				return (hard<<8)|(hard>>8);
			return hard;
		case 4: // length
			hard = dev9Ru16(addr);
			Log::Console.debug("DEV9: TX_LENGTH[{:d}]: read {:x}\n", (addr - SMAP_BD_TX_BASE) / 8, hard);
			if(dev9.bd_swap)
				return (hard<<8)|(hard>>8);
			return hard;
		case 6: // pointer
			hard = dev9Ru16(addr);
			Log::Console.debug("DEV9: TX_POINTER[{:d}]: read {:x}\n", (addr - SMAP_BD_TX_BASE) / 8, hard);
			if(dev9.bd_swap)
				return (hard<<8)|(hard>>8);
			return hard;
		}
	...
	else if (addr >= SMAP_BD_RX_BASE && addr < (SMAP_BD_RX_BASE + SMAP_BD_SIZE))
	{
		...
		case 0: // ctrl_stat
			hard = dev9Ru16(addr);
			//Log::Console.debug("DEV9: RX_CTRL_STAT[{:d}]: read {:x}\n", (addr - SMAP_BD_RX_BASE) / 8, hard);
			if(dev9.bd_swap)
				return (hard<<8)|(hard>>8);
			return hard;
		case 2: // unknown
			hard = dev9Ru16(addr);
			//Log::Console.debug("DEV9: RX_UNKNOWN[{:d}]: read {:x}\n", (addr - SMAP_BD_RX_BASE) / 8, hard);
			if(dev9.bd_swap)
				return (hard<<8)|(hard>>8);
			return hard;
		case 4: // length
			hard = dev9Ru16(addr);
			Log::Console.debug("DEV9: RX_LENGTH[{:d}]: read {:x}\n", (addr - SMAP_BD_RX_BASE) / 8, hard);
			if(dev9.bd_swap)
				return (hard<<8)|(hard>>8);
			return hard;
		case 6: // pointer
			hard = dev9Ru16(addr);
			Log::Console.debug("DEV9: RX_POINTER[{:d}]: read {:x}\n", (addr - SMAP_BD_RX_BASE) / 8, hard);
			if(dev9.bd_swap)
				return (hard<<8)|(hard>>8);
			return hard;
		}
	...
	{
		case SMAP_R_TXFIFO_FRAME_CNT:
			Log::Console.debug("DEV9: SMAP_R_TXFIFO_FRAME_CNT read 16\n");
			return dev9Ru16(addr);
		case SMAP_R_RXFIFO_FRAME_CNT:
			Log::Console.debug("DEV9: SMAP_R_RXFIFO_FRAME_CNT read 16\n");
			return dev9Ru16(addr);
		case SMAP_R_EMAC3_MODE0_L:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_MODE0_L 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_MODE0_H:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_MODE0_H 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_MODE1_L:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_MODE1_L 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_MODE1_H:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_MODE1_H 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_RxMODE_L:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_RxMODE_L 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_RxMODE_H:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_RxMODE_H 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_INTR_STAT_L:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_INTR_STAT_L 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_INTR_STAT_H:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_INTR_STAT_H 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_INTR_ENABLE_L:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_INTR_ENABLE_L 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_INTR_ENABLE_H:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_INTR_ENABLE_H 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_TxMODE0_L:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_TxMODE0_L 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_TxMODE0_H:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_TxMODE0_H 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_TxMODE1_L:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_TxMODE1_L 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_TxMODE1_H:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_TxMODE1_H 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_STA_CTRL_L:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_STA_CTRL_L 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);

		case SMAP_R_EMAC3_STA_CTRL_H:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_STA_CTRL_H 16bit read {:x}\n", dev9Ru16(addr));
			return dev9Ru16(addr);
		default:
			Log::Console.debug("DEV9: SMAP : Unknown 16 bit read @ {:X},v={:X}\n", addr, dev9Ru16(addr));
			return dev9Ru16(addr);
	}
#endif
	return rv;
...
u32 smap_read32(u32 addr)
{
	...
	{
		case SMAP_R_TXFIFO_FRAME_CNT:
			Log::Console.debug("DEV9: SMAP_R_TXFIFO_FRAME_CNT read 32\n");
			return dev9Ru32(addr);
		case SMAP_R_RXFIFO_FRAME_CNT:
			Log::Console.debug("DEV9: SMAP_R_RXFIFO_FRAME_CNT read 32\n");
			return dev9Ru32(addr);
		case SMAP_R_EMAC3_STA_CTRL_L:
			Log::Console.debug("DEV9: SMAP_R_EMAC3_STA_CTRL_L 32bit read value {:x}\n", dev9Ru32(addr));
			return dev9Ru32(addr);
		...
		case SMAP_R_RXFIFO_DATA:
		{
			...
			//Log::Console.debug("DEV9: SMAP_R_RXFIFO_DATA 32bit read {:x}\n", rv);
			return rv;
		}
		default:
			Log::Console.debug("DEV9: SMAP : Unknown 32 bit read @ {:X},v={:X}\n", addr, dev9Ru32(addr));
			return dev9Ru32(addr);
	}
}
...
void smap_write8(u32 addr, u8 value)
{
	...
	switch (addr)
	{
		case SMAP_R_TXFIFO_FRAME_INC:
			//Log::Console.debug("DEV9: SMAP_R_TXFIFO_FRAME_INC 8bit write {:x}\n", value);
			{
				dev9Ru8(SMAP_R_TXFIFO_FRAME_CNT)++;
			}
			return;
		...
		case SMAP_R_RXFIFO_FRAME_DEC:
			//Log::Console.debug("DEV9: SMAP_R_RXFIFO_FRAME_DEC 8bit write {:x}\n", value);
			counter_lock.lock();
			dev9Ru8(addr) = value;
			{
				dev9Ru8(SMAP_R_RXFIFO_FRAME_CNT)--;
		...
		case SMAP_R_TXFIFO_CTRL:
			//Log::Console.debug("DEV9: SMAP_R_TXFIFO_CTRL 8bit write {:x}\n", value);
		...
		case SMAP_R_RXFIFO_CTRL:
			//Log::Console.debug("DEV9: SMAP_R_RXFIFO_CTRL 8bit write {:x}\n", value);
		...
		case SMAP_R_BD_MODE:
			...
			{
				Log::Console.debug("DEV9: SMAP_R_BD_MODE: Byteswapping enabled.\n");
				dev9.bd_swap = 1;
			}
			else
			{
				Log::Console.debug("DEV9: SMAP_R_BD_MODE: Byteswapping disabled.\n");
				dev9.bd_swap = 0;
			}
			return;
		default:
			Log::Console.debug("DEV9: SMAP : Unknown 8 bit write @ {:X},v={:X}\n", addr, value);
			dev9Ru8(addr) = value;
			return;
	}
}
...
void smap_write16(u32 addr, u16 value)
{
	if (addr >= SMAP_BD_TX_BASE && addr < (SMAP_BD_TX_BASE + SMAP_BD_SIZE))
	{
		...
		{
		case 0: // ctrl_stat
			Log::Console.debug("DEV9: TX_CTRL_STAT[{:d}]: write {:x}\n", (addr - SMAP_BD_TX_BASE) / 8, value);
			//hacky
			dev9Ru16(addr) = value;
			return;
		case 2: // unknown
			//Log::Console.debug("DEV9: TX_UNKNOWN[{:d}]: write {:x}\n", (addr - SMAP_BD_TX_BASE) / 8, value);
			dev9Ru16(addr) = value;
			return;
		case 4: // length
			Log::Console.debug("DEV9: TX_LENGTH[{:d}]: write {:x}\n", (addr - SMAP_BD_TX_BASE) / 8, value);
			dev9Ru16(addr) = value;
			return;
		case 6: // pointer
			Log::Console.debug("DEV9: TX_POINTER[{:d}]: write {:x}\n", (addr - SMAP_BD_TX_BASE) / 8, value);
			dev9Ru16(addr) = value;
			return;
		}
		*/
	...
	else if (addr >= SMAP_BD_RX_BASE && addr < (SMAP_BD_RX_BASE + SMAP_BD_SIZE))
	{
		...
		{
		case 0: // ctrl_stat
			Log::Console.debug("DEV9: RX_CTRL_STAT[{:d}]: write {:x}\n", rx_index, value);
			dev9Ru16(addr) = value;
			if(value&0x8000)
			{
				Log::Console.debug("DEV9:  * * PACKET READ COMPLETE:   rd_ptr={:d}, wr_ptr={:d}\n", dev9Ru32(SMAP_R_RXFIFO_RD_PTR), dev9.rxfifo_wr_ptr);
			}
			return;
		case 2: // unknown
			//Log::Console.debug("DEV9: RX_UNKNOWN[{:d}]: write {:x}\n", rx_index, value);
			dev9Ru16(addr) = value;
			return;
		case 4: // length
			Log::Console.debug("DEV9: RX_LENGTH[{:d}]: write {:x}\n", rx_index, value);
			dev9Ru16(addr) = value;
			return;
		case 6: // pointer
			Log::Console.debug("DEV9: RX_POINTER[{:d}]: write {:x}\n", rx_index, value);
			dev9Ru16(addr) = value;
			return;
		}
		*/
	...
	switch (addr)
	{
		case SMAP_R_INTR_CLR:
			//Log::Console.debug("DEV9: SMAP: SMAP_R_INTR_CLR 16bit write {:x}\n", value);
			dev9.irqcause &= ~value;
			return;

		case SMAP_R_TXFIFO_WR_PTR:
			Log::Console.debug("DEV9: SMAP: SMAP_R_TXFIFO_WR_PTR 16bit write {:x}\n", value);
			dev9Ru16(addr) = value;
			return;
...
#define EMAC3_H_WRITE(name)                                   \
	...
	case SMAP_R_EMAC3_MODE0_L:
		...
		return;
	case SMAP_R_EMAC3_TxMODE0_L:
		Log::Console.debug("DEV9: SMAP: SMAP_R_EMAC3_TxMODE0_L 16bit write {:x}\n", value);
		dev9Ru16(addr) = value;
		return;
	case SMAP_R_EMAC3_TxMODE1_L:
	...
	case SMAP_R_EMAC3_TxMODE1_H:
		...
		return;
	case SMAP_R_EMAC3_STA_CTRL_H:
		Log::Console.debug("DEV9: SMAP: SMAP_R_EMAC3_STA_CTRL_H 16bit write {:x}\n", value);
		dev9Ru16(addr) = value;
		return;
		*/

		default:
			Log::Console.debug("DEV9: SMAP : Unknown 16 bit write @ {:X},v={:X}\n", addr, value);
			dev9Ru16(addr) = value;
			return;
	}
}
...
void smap_readDMA8Mem(u32* pMem, int size)
{
	if (dev9Ru16(SMAP_R_RXFIFO_CTRL) & SMAP_RXFIFO_DMAEN)
	{
		...
		Log::Console.debug("DEV9:  * * SMAP DMA READ START: rd_ptr={:d}, wr_ptr={:d}\n", dev9Ru32(SMAP_R_RXFIFO_RD_PTR), dev9.rxfifo_wr_ptr);
		while (size > 0)
		{
		...
		Log::Console.debug("DEV9:  * * SMAP DMA READ END:   rd_ptr={:d}, wr_ptr={:d}\n", dev9Ru32(SMAP_R_RXFIFO_RD_PTR), dev9.rxfifo_wr_ptr);
...
void smap_writeDMA8Mem(u32* pMem, int size)
{
	if (dev9Ru16(SMAP_R_TXFIFO_CTRL) & SMAP_TXFIFO_DMAEN)
	{
		...
		Log::Console.debug("DEV9:  * * SMAP DMA WRITE START: wr_ptr={:d}, rd_ptr={:d}\n", dev9Ru32(SMAP_R_TXFIFO_WR_PTR), dev9.txfifo_rd_ptr);
		while (size > 0)
		{
			int value = *pMem;
		...
		Log::Console.debug("DEV9:  * * SMAP DMA WRITE END:   wr_ptr={:d}, rd_ptr={:d}\n", dev9Ru32(SMAP_R_TXFIFO_WR_PTR), dev9.txfifo_rd_ptr);
