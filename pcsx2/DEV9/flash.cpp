...
u32 FLASHread32(u32 addr, int size)
{
	...
	switch (addr)
	{
		...
		case FLASH_R_CTRL:
			Log::Console.debug("DEV9: *FLASH CTRL {:d}bit read 0x{:08X}\n", size * 8, ctrl);
			return ctrl;

		case FLASH_R_ID:
			if (cmd == SM_CMD_READID)
			{
				Log::Console.debug("DEV9: *FLASH ID {:d}bit read 0x{:08X}\n", size * 8, id);
			...
			else if (cmd == SM_CMD_GETSTATUS)
			{
				...
				Log::Console.debug("DEV9: *FLASH STATUS {:d}bit read 0x{:08X}\n", size * 8, value);
				return value;
			} //else fall off
			return 0;
...
void FLASHwrite32(u32 addr, u32 value, int size)
{
	...
	switch (addr & 0x1FFFFFFF)
	{
		...
		case FLASH_R_ADDR:
			Log::Console.debug("DEV9: *FLASH ADDR {:d}bit write 0x{:08X}\n", size * 8, value);
			address |= (value & 0xFF) << (addrbyte == 0 ? 0 : (1 + 8 * addrbyte));
			addrbyte++;
			Log::Console.debug("DEV9: *FLASH ADDR = 0x{:08X} (addrbyte={:d})\n", address, addrbyte);
			if (!(value & 0x100))
			{ // address is complete
				...
				addrbyte = 0; // address reset
				{
					...
					Log::Console.debug("DEV9: *FLASH ADDR = 0x{:08X} ({:d}:{:d}:{:d}) (addrbyte={:d}) FINAL\n", address, blocks, pages, bytes, addrbyte);
				}
			}
			break;

		case FLASH_R_CTRL:
			Log::Console.debug("DEV9: *FLASH CTRL {:d}bit write 0x{:08X}\n", size * 8, value);
			ctrl = (ctrl & FLASH_PP_READY) | (value & ~FLASH_PP_READY);
			break;

		case FLASH_R_ID:
			Log::Console.debug("DEV9: *FLASH ID {:d}bit write 0x{:08X} DENIED :P\n", size * 8, value);
			break;

		default:
			Log::Console.debug("DEV9: *FLASH Unkwnown {:d}bit write at address 0x{:08X}= 0x{:08X} IGNORED\n", size * 8, addr, value);
			break;
	}
}
