...
bool LoadFontData()
{
	...
	{
		std::optional<std::vector<u8>> font_data = Host::ReadResourceFile("Roboto-Regular.ttf");
	...
	{
		std::optional<std::vector<u8>> font_data = Host::ReadResourceFile("RobotoMono-Medium.ttf");
		if (!font_data.has_value())
			return false;
	...
	{
		std::optional<std::vector<u8>> font_data = Host::ReadResourceFile("fa-solid-900.ttf");
		if (!font_data.has_value())
			return false;
...
#define DRAW_LINE(text, color) \
	...
	// if (state == System::State::Running)
	{
		...
		if (GSConfig.OsdShowResolution)
		{
			...
			text.Clear();
			text.Write("%dx%d %s %s", width, height, GetGSVideoModeString(), GetGSInterlaceString());
		...
		if (GSConfig.OsdShowCPU)
		{
			...
			{
				text.Write("EE[%d]: %.1f%% (%.2fms)", EmuConfig.Speedhacks.EECycleRate, PerformanceMetrics::GetCPUThreadUsage(),
			...
			if (THREAD_VU1)
			{
				...
				{
					text.Write("VU[%u]: %.1f%% (%.2fms)", EmuConfig.Speedhacks.EECycleSkip,
