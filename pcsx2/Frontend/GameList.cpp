...
namespace GameList
{
	...
	static bool GetExeListEntry(const std::string& path, GameList::Entry* entry);
	static bool GetIrxListEntry(const std::string& path, GameList::Entry* entry);
...
const char* GameList::EntryTypeToString(EntryType type)
{
	static std::array<const char*, static_cast<int>(EntryType::Count)> names = {
		{"PS2Disc", "PS1Disc", "Playlist", "Executable"}};
...
const char* GameList::GetGameListCompatibilityRatingString(CompatibilityRating rating)
{
	// clang-format off
	switch (rating)
	{
	case CompatibilityRating::Unknown: return "Unknown";
	case CompatibilityRating::Nothing: return "Nothing";
	case CompatibilityRating::Intro: return "Intros";
	case CompatibilityRating::Menu: return "Menus";
	case CompatibilityRating::InGame: return "Ingame";
	case CompatibilityRating::Playable: return "Playable";
	case CompatibilityRating::Perfect: return "Perfect";
	default: return "";
...
{
	static const char* extensions[] = {".bin", ".cue", ".iso", ".cso", ".chd", ".elf", ".irx", ".m3u"};
...
bool GameList::OpenCacheForWriting()
{
	...
	m_cache_write_stream = FileSystem::OpenCFile(cache_filename.c_str(), "ab");
	if (!m_cache_write_stream)
		return false;

	if (FileSystem::FTell64(m_cache_write_stream) == 0)
	{
...
void GameList::RewriteCacheFile()
{
	...
	if (OpenCacheForWriting())
	{
		...
		{
			if (!WriteEntryToCache(&it))
			{
				Console.Error("Failed to write '%s' to new cache file", it.title.c_str());
				break;
			}
		}
