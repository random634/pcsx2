...
static std::unordered_map<std::string, GameDatabaseSchema::GameEntry> gameDb;
...
static bool parseAndInsert(std::string serial, const YAML::Node& node)
{
	...
	try
	{
		...
		gameDb.emplace(std::move(serial), std::move(gameEntry));
		return true;
	}
...
static bool CheckAndLoad(const char* cached_filename, s64 expected_mtime)
{
	...
	while (FileSystem::FTell64(fp.get()) != file_size)
	{
		...
		gameDb.emplace(std::move(serial), std::move(entry));
	}
...
static std::thread s_load_thread;
...
static void Load()
{
	...
	Console.WriteLn("[GameDB] %zu games on record (loaded in %.2fms)", gameDb.size(), timer.GetTimeMilliseconds());
}

void GameDatabase::QueueLoad()
{
	std::call_once(s_load_once_flag, []() {
		s_load_thread = std::thread(Load);
	});
}
...
const GameDatabaseSchema::GameEntry* GameDatabase::FindGame(const std::string& serial)
{
	...
	auto iter = gameDb.find(serialLower);
	if (iter == gameDb.end())
	{
