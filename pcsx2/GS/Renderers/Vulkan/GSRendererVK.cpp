...
void GSRendererVK::EmulateBlending()
{
	...
	m_p_sel.bs.abe = 1;
	m_p_sel.bs.blend_index = uint8(((ALPHA.A * 3 + ALPHA.B) * 3 + ALPHA.C) * 3 + ALPHA.D);
...
void GSRendererVK::DrawPrims(GSTexture* rt, GSTexture* ds, GSTextureCache::Source* tex)
{
	...
	if (m_p_sel.ps.hdr)
	{
		...
		dev->BlitRect(rt, dRect, 0, hdr_rt, dRect, 0, false);
	}
	...
	if (m_bind_rtsample && rt)
	{
		...
		if (rt_copy)
		{
			...
			g_vulkan_dev->PSSetShaderResource(3, rt_copy);
		}
	}
	...
	if (rt_copy)
	{
		dev->PSSetShaderResource(3, nullptr);
		dev->Recycle(rt_copy);
	}

	if (hdr_rt)
	{
		const GSVector4 sRect(GSVector4(dRect) / GSVector4(rtsize.x, rtsize.y).xyxy());
		dev->StretchRect(hdr_rt, sRect, rt, GSVector4(dRect), ShaderConvert_MOD_256, false);
		dev->Recycle(hdr_rt);
	}
}
