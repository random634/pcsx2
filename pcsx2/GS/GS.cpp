...
{
	const GSVector2i res(s_gs->GetInternalResolution());
	*width = res.x;
	*height = res.y;
}
...
void GSUpdateConfig(const Pcsx2Config::GSOptions& new_config)
{
	...
	if (GSConfig.Renderer != old_config.Renderer || GSConfig.UseBlitSwapChain != old_config.UseBlitSwapChain)
	{
...
{
#ifdef PCSX2_CORE
	return Host::GetIntSettingValue("EmuCore/GS", lpKeyName, nDefault);
#else
	BuildConfigurationMap(lpFileName);
